...
`,
  ...
  async (input) => {
    // The history is automatically passed to the model by the runner,
    // and the last message from the user is treated as the new prompt.
    // The system prompt provides the context.
    return {
      history: input.history || [],
      // This prompt is empty because the user's last message is already in the history.
      prompt: ''
    };
  }
);
